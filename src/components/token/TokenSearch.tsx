  ...
  return (
    <div ref={box} className="relative w-full max-w-xs">
      <form onSubmit={(e) => { e.preventDefault(); const s = q.trim(); if (/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(s)) router.push(`/token/${s}`); else if (hits[0]) router.push(`/token/${hits[0].mint}`); }}>
        <input value={q} onChange={(e) => setQ(e.target.value)} onFocus={() => hits.length && setOpen(true)} placeholder="Look up a token: name or mint" className="w-full rounded-full border border-line bg-[#FFFFFF] px-3.5 py-2 text-sm placeholder:text-grey focus:border-ink focus:outline-none" />
      </form>
      {open && hits.length > 0 && (
        <div className="absolute right-0 top-11 z-40 w-[22rem] max-w-[90vw] overflow-hidden rounded-2xl border border-line bg-[#FFFFFF] shadow-[0_18px_50px_-8px_rgba(23,22,19,0.12)]">
          {hits.map((h) => (
            <button key={h.mint} type="button" onClick={() => { setOpen(false); setQ(""); router.push(`/token/${h.mint}`); }} className="flex w-full items-center gap-3 px-3 py-2.5 text-left transition hover:bg-card">
              ...
              {h.change24h != null && <span className={`text-xs ${h.change24h >= 0 ? "text-[#157E4E]" : "text-[#B93A16]"}`}>{h.change24h >= 0 ? "+" : ""}{h.change24h.toFixed(1)}%</span>}
            </button>
          ))}
        </div>
      )}
