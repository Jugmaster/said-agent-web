...
export async function GET(req: Request, { params }: { params: Promise<{ mint: string }> }) {
  ...
  try {
    if (!pool) {
      const gt = await fetch(`https://api.geckoterminal.com/api/v2/networks/solana/tokens/${mint}/pools?page=1`, { headers: { accept: "application/json" }, next: { revalidate: 300 } }).then((r) => (r.ok ? r.json() : null));
      pool = gt?.data?.[0]?.attributes?.address ?? null;
    }
    if (!pool) return NextResponse.json({ candles: [], pool: null }, { headers: { "Cache-Control": "public, max-age=30" } });
    const r = await fetch(
    ...
    if (!r.ok) return NextResponse.json({ candles: [], pool, error: `upstream ${r.status}` }, { status: 200, headers: { "Cache-Control": "public, max-age=15" } });
    const j = await r.json();
    const list: number[][] = j?.data?.attributes?.ohlcv_list ?? [];
    const candles = list.map((c) => c.map(Number)).sort((a, b) => a[0] - b[0]);
    return NextResponse.json({ candles, pool }, { headers: { "Cache-Control": "public, max-age=15, s-maxage=30" } });
  } catch (err) {
    return NextResponse.json({ candles: [], pool, error: err instanceof Error ? err.message : "failed" }, { status: 200 });
  }
}
